...
- [x] Chatbot fixed for the 9-module structure (2026-07-09) — worker `SYSTEM_PROMPT` and the widget's `MODULE:XX` path map still described the old 11-module build, so valve/procedural questions produced 404 links; the bot now states those topics have no module. Worker also hardened: CORS fails closed if `ALLOWED_ORIGIN` is unset, and the unauthenticated chat/quiz routes now reject requests from disallowed origins (they proxy the Anthropic API). Worker redeployed 2026-07-12 and verified live: origin enforcement returns 403, chatbot declines valve/procedural questions without dead links, and current topics map to correct display modules.
