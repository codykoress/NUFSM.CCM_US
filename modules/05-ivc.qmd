...
1. Record baseline stroke volume or cardiac output with the patient semirecumbent. Keep the measurement method and settings consistent.
2. Use the bed to lower the trunk to supine while raising the legs to about 45°. Avoid active effort, pain, or coughing; use another dynamic test if positioning is unsuitable.
3. Capture the transient peak change promptly, usually within the first minute. For LVOT Doppler, keep the same sample position and alignment and average comparable beats; account for rhythm and measurement variability.
4. Return to the original position and confirm that the measurement moves back toward baseline. Record the percentage change, not just “positive.”

A CO/SV rise around 10% is a commonly used criterion, but use a threshold validated for the measurement method and a change greater than its measurement error. A borderline VTI change is inconclusive; repeat or use a more precise method. For example, 15.0 to 15.5 cm is only about 3.3%, whereas 15 to 18 cm is 20%. A positive test indicates potential responsiveness, not that fluid is required or safe. [Original PLR study](https://pubmed.ncbi.nlm.nih.gov/16540963/), [practical PLR rules](https://pmc.ncbi.nlm.nih.gov/articles/PMC4293822/).
